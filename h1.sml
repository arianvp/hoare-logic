...
Hol_datatype
   `GCL = SKIP
        | ASSERT of 'pred    (* 'pred is a type variable, notice the 'name syntax *)
        | ASSUME of 'pred
        | SEQ    of GCL => GCL
        | ASG    of string => 'expr
   ` ;
...
val SOUND_wlp_assert_thm = prove
    (--`HOARE (ASSERT p) (wlp (ASSERT p) q) q`--,
    REWRITE_TAC [HOARE_def]
    THEN REWRITE_TAC [wlp_def]
    THEN REWRITE_TAC [exec_def]
    THEN BETA_TAC
...
(*val SOUND_wlp_seq_thm = prove
   (--`HOARE (SEQ S1 S2) (wlp (SEQ S1 S2) q) q`--,
   );*)

(* quantify *)
val lol = GEN_ALL(wlp_seq_lemma);
...
val SOUND_wlp_thm = prove
   (--`(!q. HOARE gcl (wlp gcl q) q)`--,
    ...
    [ PROVE_TAC [SOUND_wlp_skip_thm],
      PROVE_TAC [SOUND_wlp_assert_thm],
